...
/obj/machinery/autotau/Destroy()
	qdel(wires)
	wires = null
	return ..()

/obj/machinery/autotau/proc/update_recipe_list()
	if(!machine_recipes)
		machine_recipes = autolathe_recipes

/obj/machinery/autotau/interact(mob/user as mob)
	...
	if(!disabled)
		...
		var/index = 0
		for(var/datum/autotau/recipe/R in machine_recipes)
			index++
...
/obj/machinery/autotau/attackby(var/obj/item/O as obj, var/mob/user as mob)
...
/obj/machinery/autotau/attack_hand(mob/user as mob)
	user.set_machine(src)
	interact(user)

/obj/machinery/autotau/CanUseTopic(user, href_list)
	if(busy)
		to_chat(user, "<span class='notice'>The autolathe is busy. Please wait for completion of previous operation.</span>")
		return min(STATUS_UPDATE, ..())
	return ..()

/obj/machinery/autotau/OnTopic(user, href_list, state)
	set waitfor = 0
	if(href_list["change_category"])
		...
		var/datum/autotau/recipe/making
...
/obj/machinery/autotau/update_icon()
	icon_state = (panel_open ? "autolathe_t" : "autolathe")

//Updates overall lathe storage size.
/obj/machinery/autotau/RefreshParts()
	..()
	var/mb_rating = 0
	var/man_rating = 0
...
/obj/machinery/autotau/dismantle()
