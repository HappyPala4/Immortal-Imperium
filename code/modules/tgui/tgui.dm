...
/datum/tgui/proc/get_config_data()
	var/list/config_data = list(
			...
			"style"     = style,
			"interface" = interface,
			"fancy"     = GLOB.PREF_FANCY,//user.get_preference_value(/datum/client_preference/tgui_style) == GLOB.PREF_FANCY,
			"locked"    = GLOB.PREF_PRIMARY,//user.get_preference_value(/datum/client_preference/tgui_monitor) == GLOB.PREF_PRIMARY,
			"window"    = window_id,
			"ref"       = "\ref[src]",
			"user"      = list(
				"name"  = user.name,
