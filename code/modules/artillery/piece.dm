...
/obj/structure/cannon/modern/tank/attackby(obj/item/W as obj, mob/M as mob)
	if (broken && istype(W, /obj/item/weldingtool))
...
//			return FALSE
		if (do_after(M, caliber/2, M, can_move = TRUE))
			...
			//	found_loader = FALSE
			//	for (var/obj/structure/bed/chair/loader/L in M.loc)
			//	found_loader = TRUE//Move up one when re-adding.
			//	if (found_loader == FALSE && istype(src, /obj/structure/cannon/modern/tank) && !istype(src, /obj/structure/cannon/modern/tank/voyage))
			//		M << "<span class = 'warning'>You need to be at the loader's position to load \the [src].</span>"
			//		return FALSE
				M.remove_from_mob(W)
				...
				loaded = W
				M << "You load the [src]."
				playsound(loc, 'sound/machines/switch1.ogg',100, TRUE)
				return
	else if (istype(W,/obj/item/wrench) && !can_assemble)
